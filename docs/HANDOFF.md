...
**第二条**：`temperature_sampling` **不是一项普遍改进**。它的产出率取决于具体序列而非长度（hLF 前体 710 aa 跑满 10 条，更短的成熟链 691 aa 只出 4 条，决定因素是参考序列在 ADR-0001 门槛下的余量），而多样性收益与产出率强相关。**在 hLF 成熟链这条真实构建上，等子集大小对照后最高密码子相似度只差 0.43 个百分点（71.20% vs 70.77%），代价是 29 倍耗时和少一半以上候选——没有可用收益。** 收益在 HSA 成熟链（5.5 个百分点）和 hLF 前体（10.4 个百分点）上才成立。评估这条路径时必须做等子集大小对照，否则候选数少导致的配对数少会被误读成多样性收益。实测表格见 `docs/EXECUTION_PLAN.md`。
